...
\end{description}
\begin{itemize}
    \item One parent process can spawn one or many child processes and passes attributes to them during creation. 
...
\begin{enumerate}
    ...
    \item \textbf{zombie} - The process is \textsl{dead}, aka \textsl{defunt}. There is an entry in the process table, but the process takes up no reources.
\end{enumerate}
...
\begin{itemize}
    ...
    \item \texttt{top} (table of process)
\end{itemize}

\subsubsection{\texttt{ps} Commands \& Output}
\subsubsection{\texttt{top} Commands \& Output}
